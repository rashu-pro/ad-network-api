...
class BillingService
{
    ...
    {
        return $this->calculateMappingBillBetween($mapping, Carbon::parse($mapping->start_date), Carbon::parse($mapping->end_date));
    }
    ...
    public function calculateCampaignBillTillNow(Campaign $campaign): float
    {
        return $campaign->mappings->sum(function ($mapping) {
            ...
            // Limit end date to now if campaign still running
    ...
    /**
     * Core reusable billing logic for any custom time window.
     */
